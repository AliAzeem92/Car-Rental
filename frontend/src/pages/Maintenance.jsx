      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
        <div className="p-4 border-b border-gray-200 bg-gray-50">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            {title === 'Overdue Alerts' && <AlertTriangle className="w-5 h-5 text-red-600" />}
            {title} ({items.length})
          </h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600">VEHICLE</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600">MILEAGE</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600">STATUS</th>
                {showActions && <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600">ACTION</th>}
              </tr>
            </thead>
            <tbody className="divide-y">
              {items.map((item) => {
                ...
                return (
                  <tr key={item.id} className={`hover:bg-gray-50 ${item.status === 'Overdue' ? 'border-l-4 border-l-red-500' : ''}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <Car className="w-5 h-5 text-gray-400" />
                        <div>
                          <div className="text-sm font-semibold">{item.vehicle.brand} {item.vehicle.model}</div>
                          <div className="text-xs text-gray-500">{item.vehicle.licensePlate}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm">
                      {item.type === "OIL_CHANGE" ? (
                        <div>
                          <div>Current: {item.currentMileage?.toLocaleString()} km</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          {!isDeleted && !item.isCompleted && (
                            <button
                              onClick={() => {
                                const insurance = item.vehicle.maintenance.find(m => m.type === 'INSURANCE' && !m.isCompleted);
                                const oilChange = item.vehicle.maintenance.find(m => m.type === 'OIL_CHANGE' && !m.isCompleted);
                                const service = item.vehicle.maintenance.find(m => m.type === 'SERVICE' && !m.isCompleted);
                          ...
                          {isDeleted ? (
                            <button
                              onClick={() => setConfirmModal({
                                ...
                                item,
                                title: 'Restore Alert',
                                message: 'Are you sure you want to restore this maintenance alert?'
                              })}
                            ...
                            <button
                              onClick={() => setConfirmModal({
                                ...
                                item,
                                title: 'Remove Alert',
                                message: 'This will hide the alert from the active list. You can restore it later from the Deleted section.'
                              })}
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <button onClick={loadMaintenance} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium">
            Refresh
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4">
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full border rounded-lg px-4 py-2">
              <option>All</option>
          ...
          <div className="flex-1">
            ...
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="w-full border rounded-lg px-4 py-2">
              <option>All</option>
          ...
          <div className="flex-1 flex items-end">
            <button onClick={() => { setStatusFilter('All'); setTypeFilter('All'); }} className="px-4 py-2 text-sm border rounded-lg hover:bg-gray-50">
              Clear Filters
            </button>
          </div>
        </div>
      ...
      {statusFilter === 'All' ? (
        ...
        renderTable(displayItems, `${statusFilter} Maintenance`, statusFilter !== 'Completed')
      )}

      <Modal isOpen={!!editModal} onClose={() => setEditModal(null)} title="Update Maintenance">
        {editModal && (
          <form onSubmit={handleUpdate} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700">Current Mileage</label>
              <input type="number" value={editModal.currentMileage || ""} onChange={(e) => setEditModal({ ...editModal, currentMileage: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Next Oil Change (km)</label>
              <input type="number" value={editModal.nextOilChange || ""} onChange={(e) => setEditModal({ ...editModal, nextOilChange: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Next Service Date</label>
              <input type="date" value={editModal.nextService || ""} onChange={(e) => setEditModal({ ...editModal, nextService: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Insurance Expiry</label>
              <input type="date" value={editModal.insuranceExpiry || ""} onChange={(e) => setEditModal({ ...editModal, insuranceExpiry: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div className="flex gap-3 pt-3">
              <Button type="submit" loading={saving} className="flex-1">Update</Button>
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          else if (confirmModal.action === 'restore') handleRestore(confirmModal.item);
