          ...
          <div className="space-y-3">
            ...
            {upcomingList.length > 0 ? (
              upcomingList.map((res) => (
                <div
                  key={res.id}
                  className="grid grid-cols-3 gap-4 text-sm py-2 hover:bg-gray-50 rounded"
                >
                  <div className="text-gray-800">
                    {formatDate(res.startDate)} {formatTime(res.startDate)}
                  </div>
                  <div className="text-gray-800">
        ...
        <div className="space-y-3">
          ...
          {recentReservations.length > 0 ? (
            recentReservations.map((res) => (
              <div
                key={res.id}
                className="grid grid-cols-4 gap-4 text-sm py-3 hover:bg-gray-50 rounded items-center"
              >
                ...
                <div>
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[res.status] || "bg-gray-100 text-gray-700"}`}
                  >
                    {res.status}
                  </span>
                </div>
